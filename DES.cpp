...
int main(int argc, char **argv)
{
	...
	for(int keyLength = 8; keyLength <= 24; keyLength += 8)
	{
		printf("Using key length of %i bytes\n", keyLength);
		// reset input to all zeros
		memset(data, 0, 64);
	
		bool ret = encryptCBC(key, keyLength, data, dataLength, out);
		if(!ret) printf("Error during encryption\n");
		...
		ret = decryptCBC(key, keyLength, out, dataLength, data);
		if(!ret) printf("Error during decryption\n");
		...
		ret = encryptECB(key, keyLength, data, dataLength, out);
		if(!ret) printf("Error during encryption\n");
		...
		ret = decryptECB(key, keyLength, out, dataLength, data);
		if(!ret) printf("Error during decryption\n");
